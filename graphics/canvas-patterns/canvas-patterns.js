...
const pattern2 = new CanvasPattern('pattern2', (x, y) => {
    let c = x * y * Math.tan(100* x * y);
    return [0, 0, 100 + c % 150, 100 + c % 150];
});
pattern2.draw();

const pattern3 = new CanvasPattern('pattern3', (x, y) => {
    let c = x * y * Math.cos(x + y);
    return [0, 100 + c % 150, 0, c % 255];
});
pattern3.draw();

const pattern4 = new CanvasPattern('pattern4', (x, y) => {
    let c = 10 * Math.sqrt(x * x + y * y);
    return [100 + c % 155, 100 + c % 155, 0, 150];
});
pattern4.draw();

const pattern5 = new CanvasPattern('pattern5', (x, y) => {
    let c = x * Math.sin(y) * y * Math.cos(x);
    return [c % 255, c % 255, c % 255, 150];
});
pattern5.draw();

const pattern6 = new CanvasPattern('pattern6', (x, y) => {
    let c = Math.sqrt(x * Math.sin(y) * y * Math.cos(x));
    return [c % 255, c % 255, c % 255, 150];
});
pattern6.draw();

const pattern7 = new CanvasPattern('pattern7', (x, y) => {
    let r = Math.random() * x + y;
    let g = Math.random() * y + x;
    let b = Math.random() * x * y;
    return [r % 250, g % 250, b % 250, 150];
});
pattern7.draw();
